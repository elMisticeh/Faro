...
## 🌐 Scraper Multi-Portal (v2)

### Estructura de archivos

```
C:\proyectos\real-estate\
├── scraper_master.py       ← Orquestador principal (USAR ESTE)
├── scraper.py              ← Solo Inmuebles24 (versión anterior)
├── detector.py             ← Detector de oportunidades
├── scrapers\
│   ├── __init__.py
│   ├── base.py             ← Utilidades compartidas
│   ├── inmuebles24.py      ← ~1,500 listings
│   ├── vivanuncios.py      ← ~500 listings
│   ├── lamudi.py           ← ~300 listings
│   ├── icasas.py           ← ~400 listings
│   ├── metroscubicos.py    ← ~600 listings
│   ├── propiedades.py      ← ~200 listings
│   ├── mercadolibre.py     ← ~1,000 listings (API)
│   └── trovit.py           ← ~500 listings (agregador)
```

### Comandos

```powershell
# Todos los portales (tarda 2-4 horas, ~5,000+ listings)
py scraper_master.py

# Solo un portal
py scraper_master.py --portal i24
py scraper_master.py --portal ml      # Mercado Libre (API, más rápido)
py scraper_master.py --portal lamudi

# Limitar páginas para prueba rápida
py scraper_master.py --max-pags 3

# Sin correr detector al terminar
py scraper_master.py --skip-detector

# Instalar dependencia adicional para Mercado Libre (API)
pip install aiohttp --break-system-packages
```

### Portales disponibles

| Alias | Portal | Método | Listings est. |
|-------|--------|--------|---------------|
| `i24` | Inmuebles24 | Playwright | ~1,500 |
| `viva` | Vivanuncios | Playwright | ~500 |
| `lamudi` | Lamudi | Playwright | ~300 |
| `icasas` | iCasas | Playwright | ~400 |
| `mc` | Metros Cúbicos | Playwright | ~600 |
| `prop` | Propiedades.com | Playwright | ~200 |
| `ml` | Mercado Libre | API REST | ~1,000 |
| `trovit` | Trovit | Playwright | ~500 |

**Total estimado: 5,000+ listings únicos**

> **Nota**: Los portales nuevos pueden requerir ajuste de selectores la primera vez
> que los corras — cada portal tiene su HTML. Corre con `--max-pags 2` primero para verificar.
