"""
detector_anomalias.py — Detecta y marca listings con datos sospechosos
=======================================================================
Fase 1: Reglas automáticas (gratis, instantáneo)
Fase 2: Validación con Claude Haiku para casos dudosos

Uso:
    py detector_anomalias.py              # solo reglas automáticas
    py detector_anomalias.py --ia         # + validación IA en casos dudosos
    py detector_anomalias.py --reporte    # solo muestra, no modifica DB
"""

import os, re, json, unicodedata, argparse, time
import anthropic
...
def _cargar_colonias():
    ...
    if _colonias_features is not None:
        return
    geojson_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'colonias_torreon.geojson')
...
def _punto_en_poligono(lat, lng, coords):
    """Ray casting: coords es lista de [lng, lat] (formato GeoJSON)."""
    inside = False
    n = len(coords)
    j = n - 1
    for i in range(n):
...
def _get_colonia_implan(lat, lng):
    """Retorna el nombre de la colonia IMPLAN que contiene (lat, lng), o None."""
    ...
    for feat in _colonias_features:
        ...
        if geom.get('type') == 'Polygon':
            ...
            if _punto_en_poligono(lat, lng, rings[0]):
...
# Palabras que aparecen en muchos nombres distintos y no discriminan lugar
_STOPWORDS = {
...
def _tokens(nombre):
    """Palabras significativas del nombre de colonia."""
...
def _colonias_coinciden(col_db, col_implan):
    """
    True si los nombres corresponden razonablemente al mismo lugar.
    Usa overlap de tokens: 'Hacienda del Rosario' y 'Residencial Hacienda El Rosario'
    comparten {HACIENDA, ROSARIO} → misma zona.
    'Hacienda del Rosario' y 'Torreon Jardin' no comparten nada → error real.
    """
    if not col_db or not col_implan:
        return True   # sin datos suficientes → no marcar
    if _normalizar(col_db) == _normalizar(col_implan):
        return True
    ta = _tokens(col_db)
    tb = _tokens(col_implan)
    # Si alguno quedó vacío de tokens significativos → no podemos determinar → no marcar
    if not ta or not tb:
        return True
    # Basta con 1 token significativo compartido
    return bool(ta & tb)
...
def _pm2_terreno(d):
    """precio_x_m2_terreno generado por Supabase; si es null, calcular al vuelo."""
...
def _colonia_fuera_de_pin(d):
    """El pin no cae dentro del polígono IMPLAN de la colonia registrada."""
    lat = d.get('lat')
    lng = d.get('lng')
    ...
    if col_implan is None:
        return False   # fuera de cobertura IMPLAN → no marcar
    return not _colonias_coinciden(col_db, col_implan)

REGLAS = {
    # Precio/m² terreno fuera de rango (cubre todos los subtipos de terreno)
    ...
    # Precio total sospechoso para venta
...
4. ¿Hay algún otro error evidente?
...
def main():
    ...
    # Cargar colonias antes del loop (una sola vez)
    _cargar_colonias()

    # Traer todos los listings activos (paginado)
    listings, offset = [], 0
    while True:
    ...
    # Filtrar fuera del bbox ZML
    N, S, E, W = 25.75, 25.40, -103.15, -103.65
    def en_zml(d):
        lat, lng = d.get('lat'), d.get('lng')
        if not lat or not lng:
            return True   # sin coords → pasar al detector de reglas
    ...
    for listing in listings:
        ...
        elif listing.get('alerta_precio_anomalo'):
            limpios_antes += 1

    print(f"  Sospechosos detectados: {len(sospechosos)}")
    print(f"  Sin anomalías: {len(listings) - len(sospechosos)}\n")
    ...
    for s in sospechosos:
        ...
        # pm2 bajo → auto-confirmar sin IA
        ...
        # colonia_pin_mismatch → mostrar detalle útil
        if 'colonia_pin_mismatch' in reglas:
            col_implan = _get_colonia_implan(listing.get('lat'), listing.get('lng')) or '?'
            print(f"  ! COLONIA [{listing['colonia']}] -> IMPLAN: [{col_implan}] | {listing.get('link_publicacion','')[:60]}")

        if args.ia:
            print(f"  ? IA  [{listing['colonia']}] ${listing.get('precio_mxn',0):,.0f} — {motivo_auto[:50]}")
            ...
            if resultado.get('es_error') and resultado.get('confianza') in ('alta', 'media'):
                ...
                print(f"    → ERROR confirmado: {resultado.get('motivo','')[:60]}")
            else:
                falsos_positivos.append(listing['id'])
                print(f"    → OK (falso positivo): {resultado.get('motivo','')[:60]}")
        else:
            confirmados_error.append({
                'id': listing['id'], 'motivo': motivo_auto,
    ...
    print(f"\n  Errores confirmados: {len(confirmados_error)}")
    print(f"  Falsos positivos IA: {len(falsos_positivos)}")
    ...
    if not args.reporte and confirmados_error:
        ...
        print(f"  Desactivados (alta confianza): {sum(1 for e in confirmados_error if e['confianza']=='alta')}")
        print(f"  Marcados para revisión: {sum(1 for e in confirmados_error if e['confianza']!='alta')}")
    ...
    print(f"  Listings activos en DB: {total_activos.count}")
    print(f"  Con alerta anomalía:    {total_alerta.count}")
    print(f"{'='*60}\n")
